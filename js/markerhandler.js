...
AFRAME.registerComponent("markerhandler", {
  ...
  tick: function () {
    if (elementsArray.length > 1) {
      ...
      if (length === 3) {
        var marker1=document.querySelector(`#marker-${elementsArray[0].barcode_value}`);
        var marker2=document.querySelector(`#marker-${elementsArray[0].barcode_value}`);
        var marker3=document.querySelector(`#marker-${elementsArray[0].barcode_value}`);
        ...
        if(distance1<1.25 && distance2<1.25 ){
          if(compound !== undefined){
            ...
            messageText.setAttribute("#messege");
          }
        }
        messageText.setAttribute("visible",false);
  ...
  countOccurrences:function(){
    return arr.reduce((a,v)=>(v.element_name===val? a+1:a))
  },
